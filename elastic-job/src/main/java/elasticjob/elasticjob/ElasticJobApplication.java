...
//    private static CoordinatorRegistryCenter registryCenter() {        //配置zookeeper		//注册中心( CoordinatorRegistryCenter )：用于协调分布式服务		//连接Zookeeper服务器的列表. 多个地址用逗号分隔. 如: host1:2181,host2:2181
...
//        JobCoreConfiguration coreConfiguration = JobCoreConfiguration.newBuilder(jobName, cron, shardingTotalCount).build();                     // 定义SIMPLE类型配置
...
//        String jobClass = TestJob.class.getCanonicalName();
//        SimpleJobConfiguration simpleJobConfiguration = new SimpleJobConfiguration(coreConfiguration, jobClass);                              // 定义Lite作业根配置
//        // 作业分片策略:系统提供三种策略和自定义可供选择
//        /*		 * 	1. AverageAllocationJobShardingStrategy：基于平均分配算法的分片策略		 *  2. OdevitySortByNameJobShardingStrategy：根据作业名的哈希值奇偶数决定IP升降序算法的分片策略		 *  3. RotateServerByNameJobShardingStrategy:根据作业名的哈希值对服务器列表进行轮转的分片策略		 * 	4. 实现JobShardingStrategy接口，并且实现sharding的方法，		 * 		接口方法参数为【作业服务器IP列表和分片策略选项】，【分片策略选项包括作业名称】，【分片总数以及分片序列号和个性化参数对照表】，可以根据需求定制化自己的分片策略。		 * */
//        // 默认的分片策略
